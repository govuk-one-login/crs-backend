...
compress(generateByteArray(numberOfRecords, allowedBytes).join(""), 'gzip').then(function (compressedString) {
    bitStringResponse.credentialSubject.encodedList = btoa(String.fromCharCode.apply(null, new Uint8Array(compressedString)));
    encodeMessageJWT(tokenPayload).then(function (encodedMessage) {
        createFile("BitStringStatusListResponseJWT.json", encodedMessage);
    });
});
compress(generateByteArray(numberOfRecords, allowedBytes).join(""), 'deflate').then(function (compressedString) {
    ...
    encodeMessageCwt(tokenPayload).then(function (encodedMessageCWT) {
        createFile("TokenStatusListResponseCWT.json", encodedMessageCWT);
    });
});
function generateByteArray(length, validValues) {
...
async function encodeMessageJWT(message) {
...
async function encodeMessageCwt(message) {
    const headers = {
        p: {
    ...
    const cwtPayload = cbor.encode(tokenPayload);
