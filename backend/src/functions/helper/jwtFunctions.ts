...
export async function validateIssuingJWT(jwt: string,
                                         jsonPayload,
                                         jsonHeader,
                                         config: ClientRegistry): Promise<ValidationResult> {
...
export async function validateRevokingJWT(dynamoDBClient: DynamoDBClient,
                                         jwt: string,
                                         jsonPayload,
                                         jsonHeader,
                                         config: ClientRegistry): Promise<ValidationResult> {
    ...
    const commonValidationResult = await validateJWT(jwt, jsonPayload, jsonHeader, config);

    if(!commonValidationResult.isValid) {
        return commonValidationResult;
    }

    const originalIssuerResult = await verifyOriginalIssuer(dynamoDBClient, jsonPayload.uri, jsonPayload.idx, jsonPayload.iss)
...
export async function verifyOriginalIssuer(dynamoDBClient: DynamoDBClient, uri: string, idx: number, clientId: string): Promise<ValidationResult> {
    ...
    if(!originalClientId) {
        return {
            isValid: false,
            error: internalServerErrorResponse(`No client ID found on item index: ${statusListItem.idx.N} and uri: ${statusListItem.uri.S}`),
        };
    } else if (originalClientId.S != clientId) {
        logger.error("The original credential clientId is different to the clientId in the request");
        return {
            isValid: false,
