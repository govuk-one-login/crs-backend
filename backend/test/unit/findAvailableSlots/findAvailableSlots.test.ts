...
describe("Testing FindAvailableSlots Lambda", () => {
    ...
    describe("On every invocation", () => {
        it("logs STARTED message and COMPLETED message", async () => {
            ...
            expect(loggerInfoSpy).toHaveBeenCalledWith(LogMessage.FIND_AVAILABLE_SLOTS_LAMBDA_STARTED);
            expect(loggerInfoSpy).toHaveBeenCalledWith(LogMessage.FIND_AVAILABLE_SLOTS_LAMBDA_COMPLETED);
        });
    });
    ...
    describe("Happy Path Scenarios", () => {
        it("should return 200 and skip refill if both queues are at/above the target depth", async () => {
            ...
            expect(loggerInfoSpy).toHaveBeenCalledWith(LogMessage.FIND_AVAILABLE_SLOTS_LAMBDA_STARTED);
            expect(loggerInfoSpy).toHaveBeenLastCalledWith(LogMessage.FIND_AVAILABLE_SLOTS_LAMBDA_COMPLETED);
